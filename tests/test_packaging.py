    ...
    def test_build_wheel_includes_package_data_and_entry_points(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with zipfile.ZipFile(wheel_path) as zf:
                ...
                self.assertIn("pac-benchmark-v3", entry_points)
                self.assertIn("pac-benchmark-v4", entry_points)
                self.assertIn("pac-benchmark-v5", entry_points)
                self.assertIn("pac-benchmark-v6", entry_points)
                self.assertIn("pac-benchmark-full-replay", entry_points)
                self.assertIn("pac-benchmark-pooled", entry_points)
                self.assertIn("pac-dashboard", entry_points)
