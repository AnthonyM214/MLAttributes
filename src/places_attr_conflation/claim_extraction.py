...
def _identity_signal_text(text: str, attribute: str) -> float:
    ...
    if attribute == "website" and "contact" in (text or "").lower():
        base = max(base, 0.15)
    return base
...
def _claim(
    *,
    ...
    elif extraction_method == "page_extracted_value" and source_type in {"official_site", "government"}:
        if any(token in lowered_context for token in ("moved", "relocated", "formerly", "new location")):
            inferred_stale = 0.0
            inferred_identity = max(inferred_identity, 0.9)
    normalized = {
    ...
    return AttributeClaim(
        ...
        stale_signal_score=max(0.0, min(1.0, max(stale_signal_score, inferred_stale))),
